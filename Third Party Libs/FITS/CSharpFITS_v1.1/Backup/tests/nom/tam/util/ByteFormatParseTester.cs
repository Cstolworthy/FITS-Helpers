...
//Array.Clear(buffer, ' ', buffer.Length-1);
  ...
  public void testFloat()  {
	...
	for (int i=0; i<flt.Length; i += 1) {
		...
		Assertion.AssertEquals("Float check:"+i, true, float.IsNaN(chk));
	    } else if (float.IsInfinity(flt[i])) {
		Assertion.AssertEquals("Float check:"+i, flt[i], chk);
	    } else {
	        Assertion.AssertEquals("Float check:"+i, 0F, dx, 1E-6);
	    }
	    if ((i+1) % 4 == 0) {
		bp.Skip(1);
	    }
   ...
   public void testDouble()  {
	...
	for (int i=0; i<dbl.Length; i += 1) {
		...
		Assertion.AssertEquals("Double check:"+i, true, Double.IsNaN(chk));
	    } else if (Double.IsInfinity(dbl[i])) {
		Assertion.AssertEquals("Double check:"+i, dbl[i], chk);
	    } else {
	        Assertion.AssertEquals("Double check:"+i, 0F, dx, 1e-14);
	    }
  ...
  public void testBoolean() {
	...
	bp.Offset=0;
	for (int i=0; i<btst.Length; i += 1) {
	    Assertion.AssertEquals("Boolean:"+i,btst[i], bp.GetBoolean(btst.Length));
	}
    }
	...
	for (int i=0; i<100; i += 1) {
	    ...
	    Assertion.AssertEquals("String:"+i, want, s);
	    bp.Skip(1);
	}
   }
     }
